...
GLboolean fanOn = false;           // fan state
GLfloat fanAngle = 0.0;            // current rotation angle of fan blades
...
// Ceiling fan drawing function (very large blades, reduced specular)
void drawFan()
{
    // Downrod (from ceiling to motor)
    glPushMatrix();
    glTranslatef(1.5, 4.5, 7.5);      // position above bed
    GLfloat rod_ambient[] = {0.5, 0.5, 0.5, 1.0};
    GLfloat rod_diffuse[] = {0.7, 0.7, 0.7, 1.0};
    GLfloat rod_specular[] = {0.3, 0.3, 0.3, 1.0};   // reduced specular
    GLfloat rod_shininess[] = {30.0};
    glMaterialfv(GL_FRONT, GL_AMBIENT, rod_ambient);
    glMaterialfv(GL_FRONT, GL_DIFFUSE, rod_diffuse);
    glMaterialfv(GL_FRONT, GL_SPECULAR, rod_specular);
    glMaterialfv(GL_FRONT, GL_SHININESS, rod_shininess);
    glScalef(0.08, 0.5, 0.08);
    glutSolidCube(1.0);
    glPopMatrix();

    // Motor (cylindrical shape using scaled cube)
    glPushMatrix();
    glTranslatef(1.5, 4.0, 7.5);      // position above bed
    GLfloat motor_ambient[] = {0.3, 0.3, 0.3, 1.0};
    GLfloat motor_diffuse[] = {0.6, 0.6, 0.6, 1.0};
    GLfloat motor_specular[] = {0.2, 0.2, 0.2, 1.0};   // reduced specular
    glMaterialfv(GL_FRONT, GL_AMBIENT, motor_ambient);
    glMaterialfv(GL_FRONT, GL_DIFFUSE, motor_diffuse);
    glMaterialfv(GL_FRONT, GL_SPECULAR, motor_specular);
    glMaterialfv(GL_FRONT, GL_SHININESS, rod_shininess);
    glScalef(0.6, 0.3, 0.6);           // width, height, depth
    glutSolidCube(1.0);
    glPopMatrix();

    // Blades group (rotates around Y axis)
    glPushMatrix();
    glTranslatef(1.5, 4.0, 7.5);
    glRotatef(fanAngle, 0, 1, 0);      // rotate blades
    // Material for blades (light grey with very low specular)
    GLfloat blade_ambient[] = {0.8, 0.8, 0.8, 1.0};
    GLfloat blade_diffuse[] = {0.9, 0.9, 0.9, 1.0};
    GLfloat blade_specular[] = {0.1, 0.1, 0.1, 1.0};   // low specular to avoid glare
    ...
    // Blade 1 (very long: scale X = 3.0)
    glPushMatrix();
    glTranslatef(0, 0, 0);
    glScalef(3.0, 0.1, 0.45);
    glutSolidCube(0.8);
    glPopMatrix();
    // Blade 2 (rotate 90 deg)
    glPushMatrix();
    glRotatef(90, 0, 1, 0);
    glScalef(3.0, 0.1, 0.45);
    glutSolidCube(0.8);
    ...
    glPushMatrix();
    glTranslatef(1.5, 4.0, 7.5);
    GLfloat hub_ambient[] = {0.2, 0.2, 0.2, 1.0};
    GLfloat hub_diffuse[] = {0.5, 0.5, 0.5, 1.0};
    GLfloat hub_specular[] = {0.1, 0.1, 0.1, 1.0};   // low specular
...
void drawIndoorPlant(float plantX, float drawerTopY, float plantZ)
{
    ...
    glPushMatrix();
    glTranslatef(plantX, drawerTopY, plantZ);
    glRotatef(-90, 1, 0, 0);
    gluCylinder(quad, 0.22, 0.25, 0.3, 32, 32);
    glPopMatrix();
    ...
    glPushMatrix();
    glTranslatef(plantX, drawerTopY + 0.28f, plantZ);
    glRotatef(-90, 1, 0, 0);
    gluDisk(quad, 0.0, 0.22, 32, 1);
    glPopMatrix();
    ...
    glPushMatrix();
    glTranslatef(plantX, drawerTopY + 0.3f, plantZ);
    glRotatef(-90, 1, 0, 0);
    gluCylinder(quad, 0.05, 0.04, 0.4, 16, 16);
    glPopMatrix();
    ...
    for (int i = 0; i < 120; i++)
    {
        ...
        float greenVar = 0.5f + (rand() % 50) / 100.0f;
        GLfloat var_diff[] = {0.0f, greenVar, 0.0f, 1.0f};
        glMaterialfv(GL_FRONT, GL_DIFFUSE, var_diff);
...
void room()
{
    ...
    // wall besides the right wall (unchanged)
    glPushMatrix();
    glTranslatef(8,-1,0);
    glScalef(0.2, 2, 5);
    drawCube1(1, 0.8, 0.7,  0.5, 0.4, 0.35);
    ...
    // ---------- TILED FLOOR (replaces the old flat cube) ----------
    glPushMatrix();
    glScalef(5, 0.1, 7);
    glTranslatef(-1, -5, 0);
    // The top surface of the original floor cube is at local y = 3.0
    GLfloat y_top = 3.0f;
    int tilesX = 30, tilesZ = 30;
    GLfloat stepX = 3.0f / tilesX;   // local x from 0 to 3
    GLfloat stepZ = 3.0f / tilesZ;   // local z from 0 to 3
    GLfloat startX = 0.0f;
    GLfloat startZ = 0.0f;

    // Two wood/ceramic colors (light and dark brown)
    GLfloat colorA_diff[] = {0.78f, 0.57f, 0.34f, 1.0f};
    GLfloat colorA_amb[]  = {0.39f, 0.28f, 0.17f, 1.0f};
    GLfloat colorB_diff[] = {0.55f, 0.35f, 0.18f, 1.0f};
    GLfloat colorB_amb[]  = {0.27f, 0.17f, 0.09f, 1.0f};
    GLfloat specular[]    = {0.2f, 0.2f, 0.2f, 1.0f};
    GLfloat shininess[]   = {30.0f};

    for (int i = 0; i < tilesX; ++i) {
        GLfloat x0 = startX + i * stepX;
        GLfloat x1 = x0 + stepX;
        for (int j = 0; j < tilesZ; ++j) {
            GLfloat z0 = startZ + j * stepZ;
            GLfloat z1 = z0 + stepZ;
            // Checkerboard pattern
            if ((i + j) % 2 == 0) {
                glMaterialfv(GL_FRONT, GL_AMBIENT, colorA_amb);
                glMaterialfv(GL_FRONT, GL_DIFFUSE, colorA_diff);
            } else {
                glMaterialfv(GL_FRONT, GL_AMBIENT, colorB_amb);
                glMaterialfv(GL_FRONT, GL_DIFFUSE, colorB_diff);
            }
            ...
            glVertex3f(x1, y_top, z0);
            glVertex3f(x1, y_top, z1);
            glVertex3f(x0, y_top, z1);
            glEnd();
        }
    }
...
void lightTwo()
{
    ...
    GLfloat no_light[] = { 0.0, 0.0, 0.0, 1.0 };
    // Extra dim light 2
    GLfloat light_ambient[]  = {0.05, 0.05, 0.05, 1.0};   // very low ambient
    GLfloat light_diffuse[]  = { 0.1, 0.1, 0.09, 1.0 };   // very low diffuse
    GLfloat light_specular[] = { 0.05, 0.05, 0.05, 1.0 };   // very low specular
...
void display(void)
{
    ...
    bedsideDrawer();
    lamp();
    drawIndoorPlant(3.1f, 0.1f, 4.85f);
    drawIndoorPlant(1.5f, 0.1f, 10.85f);
    LinkinParkPoster();
    wallshelf();
    wardrobe();
    cupboard();
...
void myKeyboardFunc( unsigned char key, int x, int y )
{
    switch ( key )
    {
        ...
        case 'l': //to turn on and off light two
            if(switchTwo == false)
            {
        ...
        case '2': //to turn on and off lamp light
            if(switchLamp == false)
            {
...
void animate()
{
    // Fan rotation
    if (fanOn) {
        fanAngle += 5.0;  // speed of rotation
        if (fanAngle >= 360.0) fanAngle -= 360.0;
    }
    // Very slow pendulum animation (speed reduced to 1/5th)
    if(redFlag == true)
    {
        theta += 0.2;        // was 1, now super slow
        z -= 0.002;          // was 0.01, now proportional
        if(theta >= 210.0)
        {
            redFlag = false;
        }
        // optional: adjust y based on theta if needed, but not critical
...
int main (int argc, char **argv)
{
    ...
    std::cout<<"1: toggle light one (right ceiling light) on/off"<<std::endl;
    std::cout<<"2: toggle light two (left ceiling light) on/off"<<std::endl;
    std::cout<<"3: toggle lamp light on/off"<<std::endl;
    std::cout<<"f: toggle ceiling fan on/off"<<std::endl;
